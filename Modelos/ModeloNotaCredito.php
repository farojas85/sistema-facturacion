...
class ModeloNotaCredito
{
    ...
    public static function mdlMostrarNotaCredito($tabla, $item, $valor)
    {
        ...
        // MOSTRAR NOTA DE CRÉDITO
    public static function mdlMostrarNotaCreditoAnulacion($tabla, $item, $valor)
    {
        if ($item != null) {
        ...
        // MOSTRAR DETALLES DE NOTA C
        public static function mdlMostrarDetallesNc($tabla, $item, $valor)
        {
    ...
    public static function mdlDetallesNotaCreditoProductos($item, $valor)
    {
        ...
        $stmt = Conexion::conectar()->prepare("SELECT t1.idproducto, t1.cantidad,t2.descripcion, t1.valor_unitario, t1.valor_total, t1.importe_total, t1.descuento  FROM nota_credito_detalle t1 INNER JOIN productos t2 ON t1.idproducto=t2.id  WHERE $item=:$item");
        $stmt->bindParam(":" . $item, $valor, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetchall();
    ...
    public static function mdlInsertarNotaCredito($id_sucursal, $comprobante, $codigosSunat)
    {

        $stmt = Conexion::conectar()->prepare("INSERT INTO nota_credito(id, id_sucursal, tipocomp, idserie, serie, correlativo, fecha_emision, codmoneda, tipocambio, op_gravadas, op_exoneradas, op_inafectas, op_gratuitas, descuento, igv, total, codcliente, tipocomp_ref, serie_ref, correlativo_ref, seriecorrelativo_ref, codmotivo, feestado, fecodigoerror, femensajesunat, id_usuario, apertura)
    VALUES (NULL, :id_sucursal, :tipocomp, :idserie, :serie, :correlativo, :fecha_emision, :codmoneda,:tipocambio,  :op_gravadas, :op_exoneradas, :op_inafectas, :op_gratuitas, :descuento, :igv, :total, :codcliente, :tipocomp_ref, :serie_ref, :correlativo_ref, :seriecorrelativo_ref, :codmotivo, :feestado, :fecodigoerror, :femensajesunat, :id_usuario, :apertura)");
        $fechaDoc = date("Y-m-d");

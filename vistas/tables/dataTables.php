...
class DataTables
{
  ...
  public  function dtaProductos()
  {
    ...
    if ($action == 'ajax') {
      ...
      $selectnum = $_GET['selectnum'];
      $activos = $_GET['activos'];
      $selectSucursal = $_GET['selectSucursal'] ?? $_SESSION['id_sucursal'];
      ...
      foreach ($registros as $key => $value) :
        ...
        if ($value['tipocomp'] == '01') {
          ...
          if (isset($notaC['feestado']) && $value['serie_correlativo'] == $notaC['seriecorrelativo_ref']   && $notaC['feestado'] == 1) {
            ...
            $btnws = '<button class="sendwsa4" idComp="' . $value['id'] . '"></button>';
          }
        }
        ...
        echo '<td>     
                <div class="contenedor-print-comprobantes">
                    ...
                    ' . $btnAnulado . '
                    ' . $btnEnvioEmailB . '';
        if (($value['anulado'] == 'n' && empty($value['id_nc']) && ($value['feestado'] == 1 || empty($value['feestado']))) && ($value['tipocomp'] == '03' || $value['tipocomp'] == '01')) {
          echo '' . $btnws . '';
        }
        echo '</div>
  ...
  public  function dtaResumenDiario()
  {
    ...
    if ($action == 'ajax') {
      ...
      $selectSucursal = $_REQUEST['selectSucursal'] ?? $_SESSION['id_sucursal'];

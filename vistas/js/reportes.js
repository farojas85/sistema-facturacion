...
$(document).on("click", ".btnTicketR", function (e) {
 ...
 // ENVIAR REPORTES AL WHATSAPP===========================
 $(document).on("keyup", "#numberws", function (e) {
  e.preventDefault(); 
  let numberws = $("#numberws").val();  
  if(!$.isNumeric(numberws)) {
            numberws = numberws.substr(0,(numberws.length -1));
            numberws = numberws.replace(/[^0-9]/g, '');
            $('#numberws').val(numberws);
        }

 });
$(document).on("click", ".btn-enviar-ws", function (e) {
  e.preventDefault();
  let idComp = $(this).attr("idComp");
  let numberws = $("#numberws").val();
  let codnumberws = $("#codnumberws").val();
  $(".respuesta-envio-ws").hide();
  console.log(numberws, codnumberws);
  let datos = { idComp: idComp, numberws: numberws, codnumberws: codnumberws };
    ...
    success: function (data) {
      ...
      if(data != "error"){
        ...
        Swal.fire({
          icon: "success",
          title: "Ok",
          text: "¡COMPROBANTE ENVIADO AL +"+codnumberws + numberws + "!",
          //footer: '<a href>Why do I have this issue?</a>'
        });
          }else{
            $(".respuesta-envio-ws").fadeIn(500).html("¡Error al enviar, ingrese un número válido!");
            // 
          }
    },
...
$(".tablaVentas").on("click", ".sendwsa4", function (e) {
  e.preventDefault();
  let idComp = $(this).attr("idComp");

  let datos = { idComp: idComp };
  ...
  Swal.fire({
    ...
    icon: "info",
    html: `<div class="contenedor-envio-ws">
         <input type="text" class="" id="numberws" >
         <input type="hidden" class="" id="codnumberws" value="51">

         <button  class="btn btn-primary btn-enviar-ws" idComp="${idComp}"><i class="fab fa-whatsapp fa-lg"></i></button>
         
         <div class="respuesta-envio-ws"></div>
         </div>
  ...
  const input = document.querySelector("#numberws");
...
$(document).on('click', '.iti__country-list li',function(){
  $("#codnumberws").val($(this).data('dial-code'));

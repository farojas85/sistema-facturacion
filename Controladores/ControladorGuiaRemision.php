...
//  var_dump($emisor);
        ...
        $item = 'codigo';
        $valor = $datosForm['motivoTraslado'];
        $motivo_traslado = ControladorGuiaRemision::ctrMostrarTraslado($tabla, $item, $valor);

        $fecha = $_POST['fechaInicialTraslado'];
        $fecha2 = str_replace('/', '-', $fecha);
        $fechaTraslado = date('Y-m-d', strtotime($fecha2));
        $datosEnvio = array(
            'codTraslado' => $motivo_traslado['codigo'],
            'descTraslado' =>$motivo_traslado['descripcion'],
            'uniPesoTotal' => 'KGM',
